...
-- ============================================================
-- These enforce business rules at the database level.
-- Error messages are written to be human-readable so the
-- frontend can display them directly in toast notifications.

-- --------------------------------------------------------
-- 6a. WORKFLOW STATE TRANSITIONS
-- --------------------------------------------------------
...
BEGIN
  -- If linked to a request, the request must be approved
...
DECLARE
  v_role TEXT;
BEGIN
  -- Only check on status changes (ship or receive actions)
...
BEGIN
  ...
  -- Get the location type of the inventory item
...
-- --------------------------------------------------------
-- 6c. INVENTORY AUTO-ADJUSTMENT ON SHIP/RECEIVE
-- --------------------------------------------------------
